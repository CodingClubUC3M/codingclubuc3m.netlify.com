...
The ideal modelling strategy is also hugely impractical for large datasets: the full Bayesian route. In this case I will not create a `stan` program but use the `stanarm` wrapper. However, in more general problems not covered by `stanarm` one would still have to generate _ad hoc_ code.
