...
Then, here you have a summary guide of the main style syntax.
...
Besides this basics, you can to add tables, rulers, links to videos, HTML code, etc. To know more visit the creator's web site: https://daringfireball.net/projects/markdown/ or this cheatsheet https://github.com/adam-p/markdown-here/wiki/Markdown-Cheatsheet.
...
- **Interactive ducoments**: HTML Widgets, Shiny
...
The first step is get R and RStudio, and install the package *rmarkdown* with the code 
```R 
library(rmarkdown)
```
In the last versions you can directly create presentations going to `File -> New File -> R Presentation`. Then, a *.RPres* document is going to be created. This is the simplest, really simplest, way to start but my advice is go quickly to the next step if you want more flexibility in the slides and final appearance. 
...
![Markdown document of a Slidy presentation.](static/post/2019-09-24_files/image1.PNG)
...
One the header is completed, you can add any kind of content that you can practically imagine: R code, equations, charts, images, videos etc. In the next points we are going to see how to add each type of content.


### R Chunks

In addition to plain text, headers and other Markdown elements, you have the option of insert R code which will be executed every time you run the file. These parts of the document are called *R chunks*. To insert a R Chunk you can use RStudio toolbar `Insert button` or the keyboard shortcut `Ctrl + Alt + I` on Windows and `Cmd + Option + I` on macOS.
...
Then we just explored the different contents and parts of our R Markdown document. Let's see what type of output format we can obtain.
...
---
```
Moreover, you can change the presentation size, the text size, or even the transition speed in the header of the document. Specifically, for the transition speed you can set the number of second for each slide or use the standard options: `default`, `slower`, `faster`.
...
For another hand, there is a quickly way to add a background image without editting the css file,
...
Slidy has more flexibity than ioslides as to appearance and style. Now we are going to see some of the main special features that slidy has.
...
And we can adjust the font directly in the header of document without editing the css file:
...
Moreover, the syntax highlighting style can be specified with the option `highlight`. Supported styles are `default`, `tango`, `pygments`, `kate`, `monochrome`, `espresso`, `zenburn`, `haddock`, and `textmate`. And you have the option of prevent syntax highlighting passing null to the parameter.
...
Beamer themes are the same that you can find in $LaTeX$. In the next link https://hartwork.org/beamer-theme-matrix/ you have the list of the different available header options related with the appearance and style:
...
![Creating a revealjs presentation](static/post/2019-09-24_files/revealjs.png)

There are some amazing keyboard shortcuts:

-`f` for fullscreen 
- `o` or `ESC` for overview mode
- `alt` or (`crtl` in Linux) and click an element, to zoom this element
...
---
```
In revealjs you can center the text of the slides changing the `center` option to true, which by default is false, as well as the possibility of modify the transitions and backgrounds, i.e. how the slide is going to move to the next one. Available transitions and background_transitions are `default`, `fade`, `slide`, `convex`, `concave`, `zoom` or `none`. Any of this global options can be overrided specifing the data-transition attribute in the header of the slide:

```text
## Use a zoom transition {data-transition="zoom"}
```

Morevover, Revealjs lets add different backgrounds like color, image, video, and iframe:
...
Another interesting features are the great look on touch devices, the fragmented slides, easily to export to pdf, keyboard bindings, or the parallax scrolling background.

References https://CRAN.R-project.org/package=revealjs.

## Ninja presentation

The last type of presentations that we are going to see is the xaringan library. It is a R Markdown extension based on the JavaScript library remark.js (https://remarkjs.com). This package was originally designed for “ninja”, so it is recommended to people that has a well-known of CSS. For another hand, if need your slides be self-contained, then xaringan it is not a good option because needs a web server to run. Another bad news is that xaringan don't work well with HTML widgets.

To install the library type
```text
install.packages("xaringan")
```
or install it directly from github to be ensure that you are downloading the last version
```text
devtools::install_github('yihui/xaringan')
```
Once you get installed, go to the menu `File -> New File -> R Markdown -> From template` and click on ninja presentation. 

![Selecting ninja presentation.](static/post/2019-09-24_files/ninja.png)

Note: If you understand chinese you can select the last option ;).

The header is going to looks like this
```text
---
...
---
```
A lit bit more complicated than others and as you will see, there are some funny arguments that makes this library really different.
