...
or the next ones, from R code with the libraries `knitr`, `xtable`, `pander`, or `stargazer`.

```{r, echo=TRUE}
mod <- lm(dist~speed, data=cars)
knitr::kable(confint(mod), caption = 'Table with kable')
```


```{r, echo=TRUE, results='asis'}
mod <- lm(dist~speed, data=cars)
print(xtable::xtable(mod, caption = 'Table with xtable'), type = 'HTML', 
...
```{r, echo=TRUE, results='asis'}
mod <- lm(dist~speed, data=cars)
stargazer::stargazer(mod, type = 'html', title = 'Table with stargazer')
```

It is important to note that for the `xtable` and `stargazer` libraries the options *echo=FALSE* and *results='asis'* must be configured.
...
```
  ...
  `r ''````{r cars, echo = TRUE, results='asis'}
  mod <- lm(dist~speed, data=cars)
  pander::pander(mod, type = 'html', title = 'Table with pander')
  ```
...
<iframe src="https://paulalc.github.io/revealjs_example" scrolling= "yes" width="100%" height="400"></iframe>
