...
Web scraping tutorials are usually very unstable precisely because of this. To circumvent that problem, I've saved a random sample of websites for some schools in `www.buscocolegio.com` into an R package called `scrapex`. Althought the links we'll be working with will be hosted locally on your machine, the HTML of the website is identical to the one hosted on the website. 
...
If you see a lot of warnings during the installation, it is expected. Since `scrapex` is meant to store data, it weighs **more than 400MB**. Now, let's move on the fake data example and load all of our packages with:
...
knitr::include_graphics(file.path("/post/2020-02-11_files", "xml_examples/xml_one.png"))
```
...
```{r, out.width = "30%", echo = FALSE}
knitr::include_graphics(file.path("/post/2020-02-11_files", "xml_examples/xml_two.png"))
```

Similarly, you'll find that the `<carol>` tag is also matched by a `</carol>` finishing tag.

```{r, out.width = "30%", echo = FALSE}
knitr::include_graphics(file.path("/post/2020-02-11_files", "xml_examples/xml_three.png"))
```
...
Put another way, if something is nested within a **node**, then the nested node is a **child** of the upper level node. In our example, the **root** node is `<people>` so we can check which are it's children:
...
Web scraping strategies are very specific to the website you're after. You have to get very familiar with the website you're interested to be able to match perfectly the information you're looking for. In many cases, scraping two websites will require vastly different strategies. For this particular example, we're only interested in figuring out the **location** of each school so we only have to extract it's location. 

```{r, echo = FALSE, out.width = 3000}
knitr::include_graphics(file.path("/post/2020-02-11_files", "buscocolegios_xml/main_page.png"))
```
...
```{r, echo = FALSE, out.width = 3000}
knitr::include_graphics(file.path("/post/2020-02-11_files", "buscocolegios_xml/developer_tools.png"))
```
...
```{r, echo = FALSE, out.width = 3000}
knitr::include_graphics(file.path("/post/2020-02-11_files", "buscocolegios_xml/search_developer_tools.png"))
```
...
```{r, echo = FALSE, out.width = 3000}
knitr::include_graphics(file.path("/post/2020-02-11_files", "buscocolegios_xml/location_tag.png"))
```
...
```{r, echo = FALSE, out.width = 3000}
knitr::include_graphics(file.path("/post/2020-02-11_files", "buscocolegios_xml/location_tag_zoomed.png"))
```
...
* Limit your scraping to non-busy hours such as overnight. This can help reduce the chances of collapsing the website since less people are visiting websites in the evening.
